...
function parseClaudeResponse(responseText: string): AIAnalysisResponse {
  ...
  let repaired = jsonText
    ...
    .replace(/[\x00-\x1f]/g, m =>    // escape control chars
      m === '\n' ? '\\n' : m === '\r' ? '\\r' : m === '\t' ? '\\t' : '',
    );
  ...
  repaired = repaired.replace(
    /"([^"\\]*(?:\\.[^"\\]*)*)"/g,
    (match) => {
      // Replace unescaped internal quotes (not at boundaries)
  ...
  // Fourth attempt: strip refactoredCode field (often contains problematic characters)
  ...
  // Fifth attempt: extract arrays individually using balanced bracket matching
  ...
  console.error('[AI-ANALYZE] First 1000 chars:', jsonText.substring(0, 1000));
  console.error('[AI-ANALYZE] Last 500 chars:', jsonText.substring(jsonText.length - 500));
  return {
    issues: [],
    strengths: [],
