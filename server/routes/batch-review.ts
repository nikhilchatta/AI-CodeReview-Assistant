...
export interface FileToReview {
  ...
  content: string;
  language?: string;
  diff?: string; // Optional: git diff for changed lines only
}
...
export function createBatchReviewRouter(): Router {
  ...
  router.post('/review/batch', async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Step 1: Run pattern rules on all files
      ...
      // Step 2: Run AI analysis (batch mode)
      ...
      // Step 3: Calculate scores and summary
  ...
  /**
   * POST /review/pr
   * Review a pull request (accepts git diff context)
   */
  router.post('/review/pr', async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(`[BATCH-REVIEW] PR review: ${reviewableFiles.length} files (${files.filter(f => f.status === 'added').length} added, ${files.filter(f => f.status === 'modified').length} modified)`);

      // Use batch review endpoint logic
      const batchReq = {
        ...req,
        body: {
          ...
          options: {
            ...options,
            // For PRs, we want stricter defaults
      ...
      // Forward to batch handler
      return router.handle(batchReq, res, () => {});
    } catch (error: any) {
  ...
  /**
   * GET /review/health
   * Health check for CI/CD integration
   */
...
function mergeAIResults(
  ...
  for (const aiFile of aiResults.files) {
    ...
    for (const aiIssue of aiFile.issues) {
      ...
      if (existingIssue) {
        ...
        if (aiIssue.reasoning) {
          existingIssue.reasoning = aiIssue.reasoning;
        }
      } else {
    ...
    // Add strengths and recommendations
